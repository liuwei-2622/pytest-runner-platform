...
- 支持运行记录趋势统计和批量删除，便于清理长期累积的报告与日志
...
运行记录页支持勾选多条记录后批量删除。删除运行记录时会同步清理该次运行生成的报告目录：

```text
reports/<run_id>/
```

为了避免破坏正在写入的日志和报告，`queued` / `running` 状态的运行会被自动跳过，并在页面提示跳过数量。删除过程中如果报告目录异常或清理失败，页面会显示对应提示，方便后续排查。
